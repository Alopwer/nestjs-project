...
@Injectable()
export class CreateWorkspaceTransaction extends BaseTransaction<
  ExtendedCreateWorkspaceDto,
  Workspace
> {
  constructor(
  ...
  protected async execute(
    data: ExtendedCreateWorkspaceDto,
    ...
    if (data.imageObject) {
      coverImage = await this.fileService.uploadPublicFile(
        data.imageObject.buffer,
        data.title,
      );
    ...
    if (data.coworkers?.length) {
      ...
      const workspaceRelations = manager.create(
        WorkspaceRelation,
        workspaceRelationsData,
      );
